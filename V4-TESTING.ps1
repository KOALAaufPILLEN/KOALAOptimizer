﻿# KOALA Gaming Optimizer v4.0 - COMPLETE ENHANCED VERSION
...
function Apply-FallbackThemeColors {
    ...
    "ScrollViewer" {
        ...
        if ($element.Template) {
            try {
                $scrollBars = @()
                # integrate scrollbar theming when helper exists
            } catch { }
        }
    }
...
function Apply-UiScale {
    ...
    try {
        if ($global:MainWindow) {
            $global:MainWindow.LayoutTransform = New-Object System.Windows.Media.ScaleTransform($Scale,$Scale)
        }
    } catch { }
}

try {
    $src = [System.Windows.PresentationSource]::FromVisual($global:MainWindow)
    if ($src -and $src.CompositionTarget) {
        $m = $src.CompositionTarget.TransformToDevice
        $scale = [Math]::Round([double]$m.M11,2)
        if ($scale -lt 0.75) { $scale = 0.75 }
        if ($scale -gt 2.0)  { $scale = 2.0 }
        Apply-UiScale -Scale $scale
    }
} catch { }


function Start-Blink {
    param($button, [string]$ColorName = "Gray", [int]$DurationMs = 400)
    try {
        if (-not $button) { return }
        $orig = $button.Background
        $brush = [System.Windows.Media.Brushes]::$ColorName
        if ($brush) { $button.Background = $brush }
        Start-Sleep -Milliseconds $DurationMs
        $button.Background = $orig
    } catch { }
}

if (-not (Get-Command Write-UiLog -ErrorAction SilentlyContinue)) {
    function Write-UiLog {
        param([string]$Message,[ValidateSet('Info','Success','Warning','Error')][string]$Level='Info')
        try {
            if (Get-Command Log -ErrorAction SilentlyContinue) { Log $Message $Level } else { Write-Host "[$Level] $Message" }
            if ($script:BtnLog) {
                switch ($Level) {
                    'Success' { Start-Blink $script:BtnLog 'Green' }
                    'Warning' { Start-Blink $script:BtnLog 'Orange' }
                    'Error'   { Start-Blink $script:BtnLog 'Red' }
                    default   { Start-Blink $script:BtnLog 'Gray' }
                }
            }
        } catch { }
    }
